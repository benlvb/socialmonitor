...
import type { Gplay, GplayReview, ReviewsOptions, ReviewsResult } from "google-play-scraper";
import { fakeSql } from "./helpers/fake-sql";
import type { MonitorRow, TargetRow } from "../src/db/repos";
import { playstoreAdapter, setPlayScraperForTests } from "../src/adapters/playstore";
 ...
 * with no token; only app() says 404.
 */
...
const review = (id: string, date: string, extra: Partial<GplayReview> = {}): GplayReview => ({
  id,
  userName: `user ${id}`,
  date,
  score: 4,
...
const fullPage = (page: number, token: string | null) => ({
  data: Array.from({ length: 100 }, (_, k) => review(`p${page}-${k}`, iso(10_000 - page * 100 - k))),
  nextPaginationToken: token,
});

interface Scripted {
  gplay: Gplay;
  reviewCalls: ReviewsOptions[];
  appCalls: string[];
}
/** Each reviews() call consumes the next scripted result (or throws the scripted error). */
function scripted(pages: (ReviewsResult | Error)[], appLookup: "ok" | Error = "ok"): Scripted {
  const reviewCalls: ReviewsOptions[] = [];
...
describe("Google Play public transport — cursor semantics", () => {
  it("forward-only first sync validates the package with app() and fetches no reviews", async () => {
    ...
    expect(s.appCalls).toEqual(["com.realbyteapps.moneymanagerfree"]);
  ...
  it("walking back to the cursor covers the window: newer items stored, cursor = newest date, meta cleared, lang + NEWEST sent", async () => {
    ...
    expect(s.reviewCalls[0]).toMatchObject({ appId: "com.realbyteapps.moneymanagerfree", lang: "en", sort: 2, num: 100, paginate: true });
  ...
  it("page budget exhausted with pages remaining: HOLD, remember the token and the newest seen, warn once per stream", async () => {
    ...
    expect(r.cursorMeta).toEqual({ pending_token: "T2", pending_newest: iso(10_000 - 100) });
